...
object OutboxId:
  ...
  def from(raw: String): Either[ValidationFailure, Type] =
    if raw == null then Left(ValidationFailure("outboxId", "must not be null"))
    else if raw.isEmpty then Left(ValidationFailure("outboxId", "must not be empty"))
    else if raw.length > 255 then
      Left(ValidationFailure("outboxId", "must contain at most 255 characters"))
    else if raw.exists(character => character.isControl || character.isWhitespace) then
      Left(ValidationFailure("outboxId", "must not contain whitespace or control characters"))
    else Right(raw)
...
object SiteId:
  ...
  def from(raw: String): Either[ValidationFailure, Type] =
    if raw == null || raw.isEmpty then Left(ValidationFailure("siteId", "must not be empty"))
    else if raw.length > 255 then
      Left(ValidationFailure("siteId", "must contain at most 255 characters"))
    else if raw.exists(character => character.isControl || character.isWhitespace) then
      Left(ValidationFailure("siteId", "must not contain whitespace or control characters"))
    else Right(raw)
