...
object ResultAttachment:
  def attachFile[F[_]: Async, A](
    ...
    Async[F].blocking(readBounded(envelopePath, handle.maximumEnvelopeBytes)).attempt.map {
      case Right(bytes) =>
        attach(attempt, handle, contract, bytes, observedOutputs, observedAt)
      case Left(error) =>
  ...
  def attach[A](
    ...
    preflight(attempt, handle, contract) match
      case Some(diagnostic) => ExecutionResult.ResultInvalid(Diagnostics.one(diagnostic), evidence)
      case None             =>
          ...
          case Right(envelope) =>
            validateEnvelope(handle, envelope) match
              case Some(diagnostic) =>
                ExecutionResult.ResultInvalid(Diagnostics.one(diagnostic), evidence)
              case None =>
                envelope.status match
                  ...
                  case ResultEnvelopeStatus.Succeeded =>
                    validateSuccess(contract, envelope, observedOutputs, evidence)
        ...
        envelope.value match
          ...
          case Some(bytes) =>
            contract.codec.decode(bytes) match
              ...
              case Right(value)  => ExecutionResult.Succeeded(value, outputs, evidence)
  ...
  private def invalid[A](
    ...
    ExecutionResult.ResultInvalid(Diagnostics.one(Diagnostic(code, message)), evidence)
