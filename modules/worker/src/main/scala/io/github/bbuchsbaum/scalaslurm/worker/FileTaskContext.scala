...
object FileTaskContext:
  ...
  final private class FileOutputStaging(root: Path, maximumOutputCount: Int)
    ...
    def seal(
      ...
      IO.blocking {
        ...
        case TooManyFiles => Left(TaskIoFailure.TooManyOutputs(maximumOutputCount))
        case error        =>
          Left(TaskIoFailure.OutputUnavailable(fallbackOutput, safeMessage(error)))
      }

  private object TooManyFiles extends RuntimeException

  private val fallbackOutput: RelativeOutputPath = RelativeOutputPath
    .from("unknown-output")
  ...
  private def listRegularFilesBounded(root: Path, maximum: Int): Vector[Path] =
    ...
    try
      ...
      while iterator.hasNext do
        visited += 1
        if visited.toLong > maximumVisited then throw TooManyFiles
        val path = iterator.next()
        if Files.isRegularFile(path, LinkOption.NOFOLLOW_LINKS) then
          builder += path
          count += 1
          if count > maximum then throw TooManyFiles
      builder.result()
  ...
  private def deleteCreatedTree(root: Path, candidate: Path): Unit =
    ...
    if safeCandidate.getParent != safeRoot then
