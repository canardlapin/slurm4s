...
class BatchSuite extends FunSuite:
  ...
  test("a grid product too large to materialize is refused rather than attempted") {
    val wide = Axis.of(1, (2 to 2000).toVector*)
    val refused = Grid.cross(wide, wide)((left, right) => left -> right)

    assert(
      refused.left.exists(_.isInstanceOf[BatchPlanFailure.TooManyElements]),
      s"a 4,000,000-row product must be refused, observed $refused"
    )
  }

  test("an ordinary product is still built") {
    val small = Axis.of(1, 2, 3)

    assertEquals(Grid.cross(small, small)((a, b) => a -> b).map(_.size), Right(9L))
  }
