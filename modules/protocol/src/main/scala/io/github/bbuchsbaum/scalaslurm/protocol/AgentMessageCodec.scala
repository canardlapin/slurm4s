...
object AgentMessageCodec:
  private val schema = SchemaId
    .from("scala-slurm.agent-message")
    .fold(problem => throw new IllegalStateException(problem.reason), identity)
  private val reservedPayloadFields = Set("requestId", "kind", "method", "status", "body")
  ...
  def encode(message: AgentEnvelope): Vector[Byte] =
    ...
    val payload = message.extensions.toIterable.foldLeft(bodyFields) {
      case (fields, (name, value)) if !reservedPayloadFields.contains(name) =>
  ...
  def decode(bytes: Vector[Byte]): Either[AgentCodecFailure, AgentEnvelope] =
    ...
    yield AgentEnvelope(
      ...
      extensions = JsonObject.fromIterable(
        root.toIterable.filterNot(entry => reservedPayloadFields.contains(entry._1))
      )
    )
...
object HandshakeJson:
  ...
  def decodeRequest(json: Json): Either[String, HandshakeRequest] =
    for
      ...
      limit <- io.github.bbuchsbaum.scalaslurm.core.ByteLimit
        .from(maximum)
        .left
        .map(_.reason)
  ...
  def decodeResponse(json: Json): Either[String, HandshakeResponse] =
    ...
    yield HandshakeResponse(protocol, limit, features, build)
