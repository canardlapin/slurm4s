...
final class LocalLogReader[F[_]: Async](
  ...
  private def readBlocking(
    ...
    if !path.startsWith(normalizedRoot) || path == normalizedRoot then
      LogReadResult.Failed(
        Diagnostics.one(
  ...
  private def readRegularFile(
    ...
    if cursor.fileIdentity.exists(_.value != identity.value) || cursor.offset.value > size then
      ...
      val nextOffset =
        LogOffset.from(Math.addExact(cursor.offset.value, bytes.size.toLong)).toOption.get
      LogReadResult.Page(
        LogPage(
          bytes = bytes,
  ...
  private def fileIdentity(path: Path, attributes: BasicFileAttributes): FileIdentity =
    val material =
      s"${path.toString}|${Option(attributes.fileKey()).fold("none")(_.toString)}|${attributes.creationTime().toMillis}"
    ...
    val digest = MessageDigest
      ...
      .mkString
    FileIdentity.from(digest).toOption.get
