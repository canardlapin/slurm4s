    ...
    Either
      ...
      .flatMap { validPageStart =>
        ...
        val unique = patterns
          ...
          .map(_.minBy(_._2))
          .toVector
          .sortBy { case (candidate, index) => index -> candidate.kind.ordinal }
...
enum DiagnosisEvidence derives CanEqual:
  ...
  case Diagnostics(value: io.github.bbuchsbaum.scalaslurm.core.Diagnostics)
...
object FailureDiagnosis:
    ...
    NonEmptyVector.fromVector(confirmed) match
      ...
      case None =>
        val grouped = suspected
          .groupBy(_.cause)
          .toVector
          .sortBy { case (cause, values) => (-values.map(_.authority).max, cause.toString) }
  ...
  private def ordered(values: Vector[Finding]): Vector[Finding] =
    values.sortBy(value => (-value.authority, value.cause.toString))
