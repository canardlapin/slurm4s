...
object OutputManifest:
  ...
  private[scalaslurm] def verified(entries: Vector[OutputEntry]): OutputManifest =
    OutputManifest(entries.sortBy(_.path.value))
...
final case class OperationRef[I, O](
    id: OperationId,
    version: OperationVersion,
    inputSchema: SchemaId,
    outputSchema: ResultSchemaId
) derives CanEqual:
  def descriptor: RegisteredOperation =
    RegisteredOperation(id, version, inputSchema, outputSchema)

final case class RegisteredOperation(
    id: OperationId,
    version: OperationVersion,
    inputSchema: SchemaId,
    outputSchema: ResultSchemaId
