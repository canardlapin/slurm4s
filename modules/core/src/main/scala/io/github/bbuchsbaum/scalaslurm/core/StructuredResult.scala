...
object ResultEnvelope:
  ...
  private[scalaslurm] def decoded(
      submissionKey: SubmissionKey,
      attemptId: AttemptId,
      attemptEpoch: AttemptEpoch,
      job: Option[JobRef],
...
object OutputValidation:
  ...
  private def pathFailures(
    ...
    missing.sortBy(_.value).map(OutputValidationFailure.Missing.apply) ++
      unexpected.sortBy(_.value).map(OutputValidationFailure.Unexpected.apply)
  ...
  private def contentFailures(
    ...
    (reportedByPath.keySet intersect observedByPath.keySet).toVector.sortBy(_.value).flatMap {
      path =>
