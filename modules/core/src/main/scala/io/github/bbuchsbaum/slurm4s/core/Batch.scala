...
final case class Grid[A] private (rows: NonEmptyVector[A]) derives CanEqual:
  def size: Int = rows.length.toInt

  def map[B](f: A => B): Grid[B] =
    Grid(rows.map(f))

  def product[B](other: Grid[B]): Grid[(A, B)] =
    Grid(
      rows.flatMap(left => other.rows.map(right => left -> right))
    )
...
object Grid:
  ...
  def cross[A, B, Z](first: Axis[A], second: Axis[B])(f: (A, B) => Z): Grid[Z] =
    fromAxis(first).product(fromAxis(second)).map(f.tupled)
  ...
  def cross[A, B, C, Z](
    ...
    fromAxis(first)
      .product(fromAxis(second))
      .product(fromAxis(third))
...
object Argument:
  ...
  private[core] def trusted(raw: String): Type = raw
...
final case class BatchPlan[I, O] private[core] (
    task: BatchTask[I, O],
    execution: BatchExecutionPlan,
    resources: ResourceRequest,
    array: Option[JobArrayRequest],
