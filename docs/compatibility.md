...
`slurm4s-core` intentionally re-exports the provider-neutral kernel types used by its public
signatures. Those aliases preserve exact `remote-exec-kernel` type identity and are part of the
slurm4s source-compatibility surface; provider-neutral libraries should import the kernel package
directly.
...
The executable publication procedure and the current pre-baseline limitations are recorded in
...
- Canonical JSON is UTF-8 with lexicographically sorted object keys, no insignificant whitespace,
  and stable numeric/string representation. `VersionedJson` documents end with exactly one
  trailing LF; embedded canonical objects such as managed requests and journal checksums do not.
...
Unknown handshake feature names are ignored so feature negotiation itself remains extensible;
semantic additions such as termination notices are sent only after explicit negotiation.
Failure responses carry an additive `code` plus the fields needed to reconstruct the public
`AgentFailure` case, including bounded remote evidence when the failure has it. Readers still
accept legacy message-only bodies and degrade unknown future codes according to the enclosing
response status. A recognized code with missing required fields is malformed, not a legacy
fallback, and SSH transport evidence is used only when the payload supplied no remote evidence.
The named `AgentDomainJson` codec owns the P2 opaque-script scheduler schema, including every sum
type discriminator and product field. The v1 scheduler response tags intentionally retain their
historical Scala case-name spelling, but they are now string literals in explicit codecs rather
than compiler-derived names. Full local/agent differential round trips and the
`agent-scheduler-shapes-v1.json` fixture pin those bytes. Changing their JSON shape is a wire change
and requires compatibility evidence, not a mechanical refactor.
...
requires replay fixtures and migration evidence.

The file store bounds snapshot-plus-suffix storage with
`JournalLimits.maximumStorageBytes`. Compaction atomically publishes a checksummed
`slurm4s.control-snapshot` projection, then resets the forced command suffix under the same writer
lock. Recovery accepts legacy journals without a snapshot and can also recover the deliberate
intermediate state where a new snapshot exists beside the old prefix. Startup replays only records
newer than the snapshot revision. Opening an oversized legacy journal under a bounded configuration
promotes its replayed projection to a snapshot immediately, so subsequent read-only opens do not
remain O(all history).

Compaction retains the configured recent event window, not infinite event history. An event request
whose cursor predates that window returns `EventHistoryGap`; `ManagedController.eventStream` fails
with `EventHistoryUnavailable` rather than silently skipping events. This is a one-way persistence
upgrade for older slurm4s readers, which do not understand the snapshot sidecar.
...
`ResultContractDescriptor` carries only mode, schema, and declared paths. Result byte limits live
on structured contracts and durable handles, then are enforced by envelope readers; there is no
second descriptor limit that callers can set without effect. Declared-output contracts likewise
make no separate manifest-size promise: envelope and output-capture budgets own those bounds.
...
Successful accounting uses `CompletionExitStatus.ReportedZero` only when Slurm disclosed an
explicit zero; `Undisclosed` records a successful state without inventing an exit code. The v1 wire
field remains required and accepts only `0` or `null`.
A trailing `+` on a state expression is preserved as
`JobObservation.stateExpressionCompleteness = Truncated`, so the typed flag list is not mistaken
for complete. A current parser reports `Complete` when no truncation marker is present; older agent
payloads that lack the additive field decode as `Unreported`, not as known complete. Parse failure
produces failed observation evidence; it never implies workload failure.
